...
link:https://github.com/NetApp/beegfs/blob/master/getting_started/beegfs_on_netapp/gen2/H2_DAC_IB_A2_EF600/beegfs_cluster_building_blocks/beegfs_ha_building_block_cluster_1/host_vars/beegfs_01.yml["こちらをご覧ください"^]をクリックすると、単一のファイルノードを表す完全なインベントリファイルの例が表示されます。
...
NVIDIA ConnectX-Virtual Protocol Interconnect® (VPI) アダプタは、トランスポート層として InfiniBand とイーサネットの両方をサポートしています。モードの切り替えは自動的にネゴシエートされないため、 `mstconfig` に含まれるツールを使用して設定する必要があります。 `mstflint` は、link:https://network.nvidia.com/products/adapter-software/firmware-tools/["NVIDIAファームウェアツール(MFT)"^] の一部であるオープンソースパッケージです。アダプターのモード変更は一度だけ行えばよい。これは手動で行うことも、インベントリの  `eseries-[ib|ib_iser|ipoib|nvme_ib|nvme_roce|roce]_interfaces:` セクションを使用して設定されたインターフェースの一部として Ansible インベントリに含め、自動的にチェック／適用されるようにすることもできます。
