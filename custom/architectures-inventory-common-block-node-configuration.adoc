...
link:https://github.com/NetApp/beegfs/blob/master/getting_started/beegfs_on_netapp/gen2/H2_DAC_IB_A2_EF600/beegfs_cluster_building_blocks/beegfs_ha_building_block_cluster_1/group_vars/eseries_storage_systems.yml["こちらをご覧ください"^]をクリックすると、一般的なブロックノード構成を表す完全なインベントリファイルの例が表示されます。
