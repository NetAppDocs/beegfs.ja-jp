...
NOTE: パスワードをプレーンテキストで保存しないでください。代わりにAnsible Vaultを使用します（を参照してください） link:https://docs.ansible.com/ansible/latest/user_guide/vault.html["Ansibleのドキュメント"^] Ansible Vaultを使用してコンテンツを暗号化する場合）またはを使用します `--ask-become-pass` プレイブックを実行する際のオプション。
...
サポートされているパラメータの完全なリストと詳細については、link:https://github.com/NetApp/beegfs/tree/master/roles/beegfs_client["BeeGFSクライアントの完全なドキュメント"^]を参照してください。クライアントインベントリの完全な例については、link:https://github.com/NetApp/beegfs/blob/master/getting_started/beegfs_on_netapp/gen2/H2_DAC_IB_A2_EF600/beegfs_client/beegfs_client_inventory.yml["こちらをご覧ください"^]をクリックしてください。
...
クライアントプレイブックの完全な例については、link:https://github.com/NetApp/beegfs/blob/master/getting_started/beegfs_on_netapp/gen2/H2_DAC_IB_A2_EF600/beegfs_client/beegfs_client_playbook.yml["こちらをご覧ください"^]をクリックしてください。
