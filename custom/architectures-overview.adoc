...
ファイルノードはRed Hat Enterprise Linux (RHEL) 9.4を実行し、必要なパッケージ（pacemaker、corosync、fence-agents-all、resource-agents）を含むHAリポジトリにアクセスできる必要があります。例えば、RHEL 9で適切なリポジトリを有効にするには、以下のコマンドを実行します。
