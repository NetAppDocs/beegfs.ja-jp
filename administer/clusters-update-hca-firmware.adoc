...
TIP: Lenovo ConnectX-7アダプタの場合は、NVIDIAのページにあるツールをlink:https://network.nvidia.com/support/firmware/lenovo-intelligent-cluster/["OEM ファームウェア"^]使用します `mlxfwmanager_LES`。
...
NOTE: ファームウェアのアップデートによっては、アップデートを適用するために再起動が必要になる場合があります。ガイダンスについては、link:https://networking-docs.nvidia.com/mftswum/4201/mlxfwreset-loading-firmware-on-5th-generation-devices-tool#id-(4.20.1)mlxfwreset–LoadingFirmwareon5thGenerationDevicesTool-mlxfwresetLimitations["NVIDIAのmlxfwresetの制限事項"^]を参照してください。再起動が必要な場合は、アダプタをリセットするのではなく、再起動を実行してください。
...
----
.. 再起動する `eseries_nvme_ib.service` 。
+
[source, console]
----
systemctl restart eseries_nvme_ib.service
----
.. E シリーズ ストレージ アレイのボリュームが存在することを確認します。
