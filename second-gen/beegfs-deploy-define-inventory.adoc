...
NOTE: ボリュームサイズは、ストレージプール全体（ボリュームグループとも呼ばれる）に対する割合として指定されます。NetApp では、SSD のオーバープロビジョニングのための余裕を持たせるため、各プールにいくらかの空き容量を残しておくことを強くお勧めします（詳細については、 https://www.netapp.com/pdf.html?item=/media/17009-tr4800.pdf["NetApp EF600アレイの概要"^]を参照してください）。ストレージプール、 `beegfs_m1_m2_m5_m6`は、プール容量の 1% を管理サービスに割り当てます。したがって、ストレージプール、 `beegfs_m1_m2_m5_m6`内のメタデータボリュームについては、1.92TB または 3.84TB ドライブを使用する場合はこの値を `21.25`に設定し、7.65TB ドライブの場合はこの値を `22.25`に設定し、15.3TB ドライブの場合はこの値を `23.75`に設定します。
...
NOTE: 適切なサイズについては、 link:beegfs-deploy-recommended-volume-percentages.html["ストレージプールのオーバープロビジョニングの割合を推奨します"] ..
